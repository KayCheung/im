<template>
        ...
        <el-container slot="container" class="full-height">
          ...
          <el-aside width="320px" class="aside-box">
            <el-container class="full-height" direction="vertical">
              ...
              <!-- 置顶栏 -->
              <header
                v-show="loadStatus == 3 && topSessions.length > 0"
                class="subheader"
              >
                <div
                  ...
                  :key="session.id"
                  class="top-item"
                  @click="clickTab(session.id)"
                  @contextmenu.prevent="topItemsMenu(session, $event)"
                >
                  <el-tooltip
                    effect="dark"
                    placement="top-start"
                  ...
                  >
                    <div class="avatar">
                      <span v-show="!session.avatarUrl">
                        {{
                          (session.name).substr(0, 1)
                        }}
                      </span>
                      <img
                        v-show="session.avatarUrl"
              ...
              >
                <el-main class="main">
                  ...
                  <p v-show="loadStatus == 3 && sessionTotal == 0" class="empty-data">
                    暂无聊天消息
                  </p>
  
                  <p v-show="loadStatus == 3 && sessionTotal > 0" class="main-menu">
                    <span class="title">消息记录 ({{ sessionTotal }})</span>
                  </p>
                  ...
                  <template v-if="loadStatus == 3">
                    <div
                      ...
                      :class="{ active: sessionId == session.id }"
                      @click="clickTab(session.id)"
                      @contextmenu.prevent="talkItemsMenu(session, $event)"
                    >
                      <div class="avatar-box">
                        ...
                        />
                        <div
                          v-show="session.isTop == 0"
                          class="top-mask"
                          @click.stop="topChatItem(session)"
                        >
                          <i class="el-icon-top" />
                        </div>
                      </div>
                      <div class="card-box">
                        <div class="title">
                          <div class="card-name">
                            ...
                            <div v-show="session.unread" class="larkc-tag">
                              {{ session.unread }}条未读
                            </div>
                            <div v-show="session.isTop" class="larkc-tag top">
                              TOP
                            </div>
  
                            <div v-show="session.type == 22" class="larkc-tag top">
                              BOT
                            </div>
  
                            <div
                              v-show="session.type == 21"
                              class="larkc-tag group"
                            >
                              群组
                            </div>
                            <div
                              v-show="session.isRemind == false"
                          ...
                          <div class="card-time">
                            <u-time v-if="session.sendTime != null" :value="session.sendTime" />
                          </div>
                        </div>
                        <div class="content">
                          <template
                            v-if="
                              sessionId != session.id && session.draftText
                            "
                          >
                            <span class="draft-color">[草稿]</span>
                            <span>{{ session.draftText }}</span>
                          </template>
                          <template v-else>
                            <template v-if="session.senderName != null">
                              <span>{{session.senderName}}: </span>
                            </template>
          ...
          <el-main class="ov-hidden full-height no-padding">
            <WelcomeModule v-if="sessionId == null" />
            <TalkPanel
              v-else
              class="full-height"
              :params="params"
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters(['topSessions', 'sessions', 'unreadTotal', 'sessionTotal']),
      ...mapState({
        // loadStatus: state => state.talks.loadStatus,
        loadStatus: state => state.session.loadStatus,
        // talks: state => state.talks.items,
        sessions: state => state.session.sessions,
        sessionId: state => state.dialogue.index_name,
      }),
      // 计算置顶栏目的高度
      subHeaderPx() {
        const n = 7 // 一排能显示的用户数
